...
An accepted record is immutable (§ 9). Against an accepted record, most of the
judgments below demand an edit that is not available.
...
The decision-log table these records replaced recorded reversals by overwriting
the rows they reversed (§ 9). "The old reasoning is wrong, so fix it" is the
defect, not the fix. ADR-0023 keeps a convergence claim that did not survive,
and says so.

## 2. The supersede protocol

Both directions, always (`_template.md`). Check each:
...
- Old record: `Status:` → `superseded`, `Superseded by:` filled, **body
  ...
  written into the tree in one commit, so the older body was never touched;
  check when the file was created before treating this as violated.
...
- **§ 1 vendor neutrality is suspended.** A vendor name, a vendor setting key
  and client-library behavior are not defects in a record.
- **§ 7 present tense and "never a changelog" is suspended.** Past tense, a
  date, "this replaced X" and "at the time this was believed" are all correct
  here.
...
shape are unanimous the other way" rather than naming three products. Never
raise a vendor name in an accepted record; when *authoring* one, reach for the
neutral form first.
...
- **Usage guidance.** § 1's closing paragraph and § 9 both draw this line: a
  ...
  key table, a YAML block written to be pasted, or "to enable this, set…" makes
  the record the wrong kind of document. Fix: a link from
...
- **`Consequences` with no `Bad, because`**; `Considered options` padded with an
  option that was never a candidate, or missing the "do nothing" option.
- **A figure with no provenance** in `Evidence` (§ 7's last bullet, reached by
  step 1). Records hold the figures a decision rests on. A record resting on
  no measurement deletes the heading rather than emptying it (`_template.md`).
- **An unfalsifiable claim the decision rests on.** § 7's provenance rule is
  worded for figures, and the practice just above pushes outside evidence into
  prose that names nothing, as in "mature implementations are unanimous" or
  "the standard approach is". Stating it in our own terms is right; leaving it
  unfalsifiable is not. A claim carrying a pillar of the decision says what it
  rests on, exactly as a number would.
...
formatting choice, a fix with one sensible shape, and the one most often missed,
...
a nit. The rarer, worse miss is a change that clears the bar and lands without a
record, usually one replacing an old design.
...
decision landed, and everything in §§ 1–6. It never reads prose for meaning. A
gate with opinions about wording would demand edits to immutable files.
...
2. Write `Considered options` before `Decision outcome`. It is the first thing
   lost when a record is written from memory.
...
4. Add the `README.mdx` row by hand. The scaffold does not add it.
