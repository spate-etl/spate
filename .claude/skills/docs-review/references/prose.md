...
the checklist first finds items on it instead of defects.
...
the rustdoc, or the source). Those marks are the report. Everything below is
...
does. It reads as helpful and costs the reader the thing they came for. Delete
the paragraph and ask whether the page still answers its title. If it does, the
paragraph was ballast.
...
it reads as *known and wrong*, and the reader does not stop.
...
**Explanation inside a tutorial.** The commonest failure in the whole tree. A
tutorial paragraph beginning "The reason for this is…" belongs in `02-concepts/`
with a link. The reader is mid-task; they cannot spend attention on why.
...
is true, because a reader downstream cannot check it or tell when it expired.
...
provenance, or it is a claim the project cannot stand behind. The words make the
claim themselves, so this defect can be caught by the wording alone. The rest of
the catalog needs judgment.

**A hedge standing in for an answer.** "may", "should generally", "in most
cases" where the behavior is deterministic. The reader needs to know what
happens; if it depends, say what it depends on.

## The sentence works against the reader

**Buried lede.** The page's answer sits in paragraph three. Ask of the first
screen: is this what the title promised?
...
sentence. It was carrying nothing.

**Conditions after the instruction** (§ 7). A reader who acts on the first half
has already acted.

**Future tense for current behavior** (§ 7). "The sink will retry" describes
behavior the sink already has.
...
*Not this:* a number the source closes, such as an enum's variants or a `const`
array, which step 1 verifies instead. Nor a number that *constrains* the set:
§ 3's "exactly three forms, no others" is the rule, and deleting the number
deletes it.
...
form. The build catches a path that does not resolve. It cannot catch a path
that resolves and was never made a link.

*Not this:* a path in a fenced block, or in `CONTRIBUTING.md`, `DEVELOPING.md`,
`AGENTS.md` and `docs/STYLE.md` itself. Those are contributor files, unpublished
and read from a checkout.

**First-person plural** (§ 7). "we recommend", "our design".

**Terminology drift.** The same thing under two names across one page, such as
"worker" and "instance", "chunk" and "batch", "split" and "work unit". § 7 says
define once and stay consistent; a synonym reads as a second concept.

**Admonition misuse** (§ 7). Two stacked; one opening a section; `:::danger` on
something that costs time rather than data. That spends the one form reserved
for data loss and leaves nothing louder for the case that costs data.

## Not defects here

Do not report these. Several have measured false-positive rates worse than 4:1
in the upstream style packages they come from.
...
- **Passive voice**, as a blanket rule. § 7's own exemplar is passive ("Unknown
  ...
  it adds nothing. Raise passive voice *only* where the actor is missing and the
  reader needs it, as in "the offsets are committed" on a page whose subject is
  which component commits them.
- **Adverbs and weasel words** as a category hunt.
- **Sentence length.** One idea per sentence is the rule; length is a poor
  proxy for it. A long sentence carrying one idea is fine.
- **Contractions**, either way.
- **Oxford commas, spacing, quote style, en- versus em-dashes.** Style, and not
  worth the attention budget.
...
- **Repetition across pages.** Documentation is allowed to repeat itself; DRY is
  a code rule. § 1's one-home rule governs *vendor facts* specifically, not
  prose generally.
