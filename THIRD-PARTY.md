...
This table is post-processed. `cargo-about` emits one row per licence *text*,
grouping crates by which text they share, and a crate that ships several files
scanning as the same licence has its text chosen by the order those files are
...
above from what is left. The elected licence ids are unaffected either way.
