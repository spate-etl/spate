...
//!
//! Fifteen lines of SplitMix64 rather than a dependency on `rand`, and the
//! reason is the whole point of this harness: an A/B run compiles two trees
//! that may straddle a dependency bump, and `rand`'s generators have changed
//! their output stream across majors before. A corpus that differs between the
//! legs makes every number incomparable while looking perfectly healthy — the
//! digest in [`crate::Corpus`] would catch it, but only after the run. A
//! generator whose output is pinned by a known-answer test in this file cannot
...
impl SplitMix64 {
    ...
    ///
    /// Lemire's multiply-shift reduction, without the rejection loop: the bias
    /// is below one part in 2^64 divided by `n`, which no corpus this harness
    /// generates can observe, and a rejection loop would make the number of
    /// draws depend on the value drawn — a needless way for two legs to consume
    /// the stream at different rates if the loop ever changed.
...
mod tests {
    ...
    ///
    /// This is the test that makes the hand-rolled generator worth having: it
    /// pins the output stream to three constants a reader can check against the
    /// reference implementation, so a corpus generated by any two builds of
    /// this crate is the same corpus.
    #[test]
