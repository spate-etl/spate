...
fn main() {
    ...
    let mut walls: Vec<Vec<f64>> = vec![Vec::with_capacity(reps as usize); corpora.len()];
    for _rep in 0..reps {
        for (i, corpus) in corpora.iter().enumerate() {
            walls[i].push(run_once(corpus));
        }
    }
    ...
    for (corpus, samples) in corpora.iter().zip(&walls) {
        ...
        Report::measurement(&bench)
            ...
            .metric(
                "wall_s",
                Metric::minimize(mean, "s").with_ci(lo, hi).with_n(n),
            )
            .metric(
                "wall_median_s",
