//! Zero-copy acceptance test for the fast backend's flagship shape: one
//! Avro datum carrying a sensor batch (an array of events), decoded
//! **borrowed** by `AvroFastDeserializer`, exploded with `flat_map`, and
...
//!
//! Driven exactly the way a framework user's test drives the etl-test
//! mocks (see etl-test's own contract tests): memory source → lane poll →
...
use etl_core::checkpoint::{AckRef, Checkpointer};
use etl_core::deser::{Deserializer, EmitRecord, RecFamily};
use etl_core::error::{DeserError, SinkError};
use etl_core::ops::{ChunkConfig, Emitter, PushOutcome, chain};
use etl_core::record::{PartitionId, RawPayload, Record};
use etl_core::sink::{RowEncoder, ShardRouter, shard_queues};
...
/// Wraps the fast deserializer to record each payload's buffer address range
/// before delegating.
#[derive(Clone)]
struct RangeTap<D> {
    inner: D,
...
impl ShardRouter for ToZero {
    fn route(&self, _m: &etl_core::record::RecordMeta, _n: usize) -> usize {
        0
    }
}
...
fn borrowed_batch_explodes_zero_copy_through_the_chain() {
    ...
    // The fast borrowed deserializer, from ordinary settings (raw framing:
    ...
    // Drive it the way the runtime drives a source: etl-test memory source,
