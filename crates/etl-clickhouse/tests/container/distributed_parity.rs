...
// still works. The in-cluster hostnames are therefore `etl-ch0-<nonce>` /
// `etl-ch1-<nonce>` rather than a fixed `ch-shard-0`.

use super::*;
use etl_clickhouse::{ClickHouseEncoder, DistributedRouter, ShardKey};
...
use etl_core::sink::{DrainReport, SinkPool, shard_queues};
...
fn explode(batch: SensorBatch, em: &mut Emitter<'_, Owned<EventRow>>) {
    for (name, value) in batch.events {
        ...
        // (mirroring the etl-avro flat_map template) the Flow is ignored.
...
async fn two_node_cluster(pw: &str) -> Cluster {
    let net = unique("etl-parity-net");
    let h0 = unique("etl-ch0");
    let h1 = unique("etl-ch1");
