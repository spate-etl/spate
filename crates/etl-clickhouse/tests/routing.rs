...
use etl_clickhouse::{DistributedRouter, ShardKey};
use etl_core::backpressure::InflightBudget;
use etl_core::checkpoint::AckRef;
use etl_core::deser::{Deserializer, EmitRecord, Owned};
use etl_core::error::{DeserError, ErrorClass, SinkError};
use etl_core::ops::{ChunkConfig, PushOutcome, chain};
use etl_core::record::{PartitionId, RawPayload, Record};
use etl_core::sink::{EncodedChunk, RowEncoder, shard_queues};
...
impl RowEncoder<Owned<SensorRow>> for RowBinaryEncoder {
    fn encode<'buf>(
        ...
        etl_clickhouse::serialize_row(&rec.payload, buf).map_err(|e| SinkError::Client {
