...
pub(crate) fn desired_assignment(
    ...
    // planner-supplied and unbounded (`etl-s3` reports bytes), and a leader
...
/// weights — `etl-s3` reports bytes — so `weight * load` overflows `u64`
...
mod tests {
    ...
    fn byte_scale_weights_do_not_overflow_the_improving_move() {
        // `etl-s3` reports weight in BYTES, and any object at or above the
