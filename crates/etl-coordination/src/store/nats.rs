...
//!
//! - `etl_coordination_{job}_state` — durable: no age limit; split
//!   records and the plan record survive owner death.
//! - `etl_coordination_{job}_lease` — ephemeral: bucket-level
...
async fn connect(config: &NatsConfig, lease_ttl: Duration) -> Result<Buckets, StoreError> {
    ...
    let state = ensure_bucket(
        &jetstream,
        kv::Config {
            bucket: format!("etl_coordination_{}_state", config.job),
            description: "etl-rs coordination: durable split and plan records".into(),
            history: 1,
    ...
    let lease = ensure_bucket(
        &jetstream,
        kv::Config {
            bucket: format!("etl_coordination_{}_lease", config.job),
            description: "etl-rs coordination: ephemeral lease keys".into(),
            history: 1,
            max_value_size: MAX_VALUE_BYTES,
            num_replicas: config.replicas,
            max_age: lease_ttl,
