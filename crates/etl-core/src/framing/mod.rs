...
//! source — object-storage backfills (`etl-s3`) today, HTTP chunked bodies,
...
//!
//! `etl-core` owns only the seam here — the [`RecordFramer`] trait, the
...
//! `etl-json` provides newline-delimited framing for NDJSON, a future
//! `etl-avro` an object-container block framer, and so on. A source stays
...
//! at-least-once resume; see the `etl-s3` offset model). Compression is *not*
...
///   an `etl-s3` backfill framing each object with the format's framer). The
...
///
/// Concrete implementations live in the format crates (`etl-json`'s NDJSON
/// framer, a future `etl-avro` container framer, ...). Implementations **must
