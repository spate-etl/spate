...
//! automatically and whose names are auto-prefixed `etl_<namespace>_`, so a
//! connector's or pipeline author's own series sit under the same `etl_`
//! umbrella as the framework's and join cleanly in a query.
//!
//! You pass the metric's **local name** — `"schema_fetches_total"`, not
//! `"etl_kafka_schema_fetches_total"`; the `Meter` adds the umbrella and
//! namespace for you. That is the whole point: one greppable `etl_` root for
...
//! # Namespaces
//!
//! - [`Meter::new`] uses the `custom` namespace → `etl_custom_*`. This is the
//!   bucket for pipeline-author metrics.
//! - [`Meter::with_namespace`] takes a segment a connector owns (`"kafka"` →
//!   `etl_kafka_*`). A connector that can appear at both ends of a pipeline (a
//!   Kafka source *and* a future Kafka sink) is separated by the runtime into
//!   `etl_kafka_source_*` / `etl_kafka_sink_*` when it injects the component's
...
/// (`etl_kafka_source_*` vs `etl_kafka_sink_*`). Not public: the runtime
...
/// `etl_<namespace>_` name prefix applied to every metric it mints.
///
...
/// // prepends `etl_custom_`, so this registers `etl_custom_orders_enriched_total`.
...
pub struct Meter {
    ...
    /// The full `etl_<namespace>_` prefix prepended to every metric name.
    prefix: String,
}

impl Meter {
    /// A scope for **pipeline-author custom metrics**: names land under the
    /// `etl_custom_` bucket. `component` is the instance id (e.g. `enrich`);
    ...
    /// A scope under a specific `etl_<namespace>_` bucket — for a connector
    /// that owns a segment (e.g. `"kafka"` → `etl_kafka_*`).
    ///
    /// # Panics
    ///
    ...
    /// `etl_<component_type>_<role>_*`. The `component_type` is also the
    ...
    pub(crate) fn for_component(
        ...
        // component's families never share the `etl_custom_` space with a
    ...
    /// Fully-qualify a local metric name under this scope's `etl_<namespace>_`
    /// prefix. Panics if the caller already included an `etl_` prefix — pass
    /// the local name only (`"schema_fetches_total"`, not
    /// `"etl_kafka_schema_fetches_total"`).
    fn qualify(&self, name: &str) -> SharedString {
        ...
        // and sink families (`etl_<ns>_source_*` / `_sink_*`), so a hand-written
    ...
    /// (auto-prefixed `etl_<namespace>_`); it carries the three standard
