...
/// logs; the exact drop count is always in `etl_json_deser_records_dropped_total`.
...
mod tests {
    ...
    fn skipped_records_are_counted_in_metrics() {
        ...
        let needle =
            format!(r#"etl_json_deser_records_dropped_total{{{STD},reason="malformed"}} 2"#);
        assert!(
