...
//! [`RecordFramer`](etl_core::framing::RecordFramer): it cuts a byte *stream*
//! (an object streamed by `etl-s3`, an HTTP body, a file tail) into
...
//!
//! ```no_run
//! # use etl_json::NdjsonFramer;
//! # struct S3Source;
//! # impl S3Source {
//! #     fn with_framer<F: Fn() -> Box<dyn etl_core::framing::RecordFramer> + Send + Sync>(self, _: F) -> Self { self }
//! # }
//! # let source = S3Source;
...
//! (the [`PerRecord`](etl_core::framing::FramingContract::PerRecord) contract).
