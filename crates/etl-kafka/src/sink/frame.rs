...
//! ([`SealedBatch::frames`](etl_core::sink::SealedBatch)) from pipeline
...
//! contract [`EncodedChunk`](etl_core::sink::EncodedChunk) requires for
...
mod tests {
    ...
    fn round_trips_key_headers_payload() {
        ...
        let headers = [("trace", &b"abc123"[..]), ("origin", &b"etl"[..])];
        write_message(
            &mut buf,
            Some(b"key-1"),
            headers.iter().copied(),
        ...
        assert_eq!(
            msg.headers,
            vec![("trace", &b"abc123"[..]), ("origin", &b"etl"[..])]
        );
