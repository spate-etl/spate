...
//!
//! ```sh
//! cargo test -p etl-kafka --test sink_kafka_broker -- --ignored
//! ```
...
use etl_test::{PipelineRun, wait_until};
...
fn kafka_to_kafka_split_round_trip() {
    ...
    let sink_a = etl_kafka::sink::build(sink_config(&brokers, A_TOPIC)).expect("sink a");
    let sink_b = etl_kafka::sink::build(sink_config(&brokers, B_TOPIC)).expect("sink b");
