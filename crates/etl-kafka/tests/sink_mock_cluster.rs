...
//! and enabled statistics populate the fixed `etl_kafka_sink_*` families.
//!
...
use etl_kafka::sink::{KafkaEncoder, KafkaMessage, KafkaSink, KafkaSinkConfig, MessageEncoder};
use etl_test::{PipelineRun, memory_source, wait_until};
...
fn sink(brokers: &str, extra: &str) -> KafkaSink {
    etl_kafka::sink::build(sink_config(brokers, extra)).expect("sink build")
}
...
fn spawn_pipeline(
    ...
    etl_test::SourceHandle,
    etl_core::pipeline::ShutdownHandle,
    PipelineRun,
) {
...
fn statistics_populate_kafka_sink_fixed_families() {
    ...
    metrics::with_local_recorder(&recorder, || {
        sink.writer
            .attach_metrics(Some(etl_core::metrics::Meter::with_namespace(
                "kafka",
                "orders",
                "orders_out",
                "kafka",
    ...
    wait_until(Duration::from_secs(10), "fixed families rendered", || {
        ...
        rendered.contains("etl_kafka_tx_requests_total")
            && rendered.contains("etl_kafka_produce_queue_messages")
    });
}
