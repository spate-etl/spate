...
//! sink) around an [`S3Source`](etl_s3::S3Source) pointed at `file://`
...
fn plan_only_run(fx: &Fixture, store: &etl_coordination::store::memory::MemoryStore) {
    let l = launch_tuned(
        &fx.config_yaml(""),
        test_options(),
        store,
...
fn split_progress(store: &etl_coordination::store::memory::MemoryStore) -> Vec<serde_json::Value> {
...
fn assert_stalls(fx: &Fixture, store: &etl_coordination::store::memory::MemoryStore) {
    let l = launch_tuned(
        &fx.config_yaml(""),
        test_options(),
        store,
