...
use etl_core::coordination::{CoordinationErrorKind, SplitSpec};
use etl_core::pipeline::ExitState;
use etl_s3::{SplitDescriptor, split_id_for};
use etl_test::{WriteOutcome, scripted_coordinator, wait_until};
use std::fs;
use std::path::Path;
...
fn spec_over(data: &Path, names: &[&str]) -> SplitSpec {
    let entries: Vec<etl_s3::DescriptorObject> = names
        .iter()
        .map(|name| etl_s3::DescriptorObject {
            key: data
                .join(name)
                .to_string_lossy()
                .trim_start_matches('/')
...
fn launch_scripted_coordinator(
    yaml: &str,
    coordinator: etl_test::ScriptedCoordinator,
    pre: impl FnOnce(&etl_test::SinkScript),
) -> Launched {
...
fn a_missing_object_reports_the_split_as_failed() {
    ...
    let ghost_descriptor = SplitDescriptor::new(vec![etl_s3::DescriptorObject {
        key: data
            .join("ghost.ndjson")
            .to_string_lossy()
            .trim_start_matches('/')
