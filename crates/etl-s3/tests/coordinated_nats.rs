...
//!
//! ```sh
//! cargo test -p etl-s3 --test coordinated_nats -- --ignored --test-threads=1
//! ```

mod support;

use etl_coordination::store::nats::{NatsConfig, NatsStore};
...
fn durable_resume_across_restart_loses_nothing() {
    let gw = Gateway::start("etl-s3-nats-resume");
...
fn two_instances_over_real_nats_complete_collectively() {
    let gw = Gateway::start("etl-s3-nats-pair");
    let (_nats, nats_port) = start_nats();
