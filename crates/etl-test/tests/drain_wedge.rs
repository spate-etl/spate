...
//!
//! The unit tests in `etl-core` cover `SinkPool::drain` in isolation, and the
//! `etl` facade covers the real thing against a paused ClickHouse — but that
...
use etl_core::pipeline::{ExitState, Pipeline, RuntimeOptions, SinkOptions};

use etl_core::record::PartitionId;
use etl_core::sink::{BatchConfig, InflightConfig, KeyHashRouter, SinkPoolConfig};
