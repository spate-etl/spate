//! Contract tests for the etl-test mocks, driven exactly the way a
...
fn opened_source() -> (etl_test::MemorySource, etl_test::SourceHandle, Checkpointer) {
...
fn assigned_lanes(
    source: &mut etl_test::MemorySource,
    handle: &etl_test::SourceHandle,
    lanes: &[(LaneId, PartitionId)],
) -> Vec<etl_test::MemoryLane> {
...
async fn endpoint_scripts_take_precedence_over_global_fifo() {
    ...
    match err {
        SinkError::Client { class, reason } => {
            assert_eq!(class, etl_core::error::ErrorClass::Fatal);
    ...
    match err {
        SinkError::Client { class, reason } => {
            assert_eq!(class, etl_core::error::ErrorClass::Retryable);
...
fn record(payload: &[u8]) -> etl_core::record::Record<Vec<u8>> {
    let (ack, _rx) = etl_core::checkpoint::AckRef::test_pair();
...
fn deserializer_modes() {
    let (ack, _rx) = etl_core::checkpoint::AckRef::test_pair();
    let raw = etl_core::record::RawPayload {
        bytes: b"a,b,,c",
        key: None,
        partition: P0,
        offset: 5,
