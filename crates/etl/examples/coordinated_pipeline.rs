...
//!
//! ```sh
//! cargo run -p etl --features coordination --example coordinated_pipeline
//! ```
...
use etl::coordination::driver::{CoordinationDriver, SplitOpening, SplitSource};
...
use etl::error::SourceError;
use etl::prelude::*;
use etl::record::RawPayload;
use etl::source::{LaneId, PayloadBatch, Source, SourceCtx, SourceEvent, SourceLane};
use etl_test::{TestDeserializer, TestEncoder, capture_sink, decode_rows};
    ...
    fn open_split(&mut self, opening: SplitOpening<'_>) -> Result<LedgerLane, SourceError> {
        ...
        let (start, end) = descriptor
            ...
            .ok_or_else(|| SourceError::Client {
                class: etl::error::ErrorClass::Fatal,
