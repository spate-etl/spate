...
//!
//! Operators in `etl` are **stateful closures** over the push model: `map`,
...
//! `StageLifecycle` traits in [`etl::ops`] are public for advanced stages,
...
use etl::ops::{ChunkConfig, PushOutcome, chain_owned};
use etl::record::PartitionId;
use etl::sink::{KeyHashRouter, shard_queues};
use etl::source::{LaneId, Source, SourceCtx, SourceEvent, SourceLane};
use etl_test::{TestDeserializer, TestEncoder, memory_source};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while let Ok(chunk) = receivers[0].try_recv() {
        rows.extend(
            etl_test::decode_rows(&chunk.frame)
                .into_iter()
