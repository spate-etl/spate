...
//! (`etl_json_deser_records_dropped_total`) and a filtered record dropped,
//! while the source offset only commits once every surviving record is durably
//! written (at-least-once). Runs anywhere via `etl-test`'s in-memory pieces:
//!
//! ```sh
//! cargo run -p etl --example json_ndjson_memory
//! ```
...
use etl_test::{TestEncoder, capture_sink, memory_source};
