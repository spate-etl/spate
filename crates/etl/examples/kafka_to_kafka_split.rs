...
//! [`split`](etl::ops::ChainBuilder) terminal — one Kafka sink per
//! destination topic, each a full sink with its own producer, batching,
//! and `etl_sink_*` series. Records with an unknown prefix follow the
...
//!
//! ```sh
//! cargo run --release -p etl --example kafka_to_kafka_split --features kafka
//! ```

// Examples talk to their user on stdout/stderr by design.
#![allow(clippy::print_stdout, clippy::print_stderr)]

use etl::deser::BytesPassthrough;
use etl::kafka::sink::KafkaBytesEncoder;
use etl::kafka::{KafkaSource, KafkaSourceConfig};
use etl::prelude::*;
use std::path::Path;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let config_path = std::env::var("ETL_CONFIG")
        .unwrap_or_else(|_| "crates/etl/examples/kafka_to_kafka_split.yaml".to_string());
    ...
    let eu_sink = etl::kafka::sink::from_component_config(pipeline.config().sink_config("eu")?)?;
    let us_sink = etl::kafka::sink::from_component_config(pipeline.config().sink_config("us")?)?;
