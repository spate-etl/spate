...
//! backend (e.g. `etl-coordination`'s NATS JetStream store) both
//! instances would point at in production — swap the store constructor
//! and this is the real multi-process deployment.
//!
//! ```sh
//! cargo run -p etl --features s3,json,coordination --example s3_coordinated_backfill
//! ```
