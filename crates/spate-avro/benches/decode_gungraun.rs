...
//! shared CI runners where a wall-clock number is only noise. Five cases, not
//! the full criterion matrix — callgrind runs the workload under emulation,
//! and the published comparison corpus (200 batches, 20,000 events per
//! iteration) is far too large to emulate.
//!
...
//!   exists to settle, and instructions attribute it without a timer.
...
main!(library_benchmark_groups = decode);
