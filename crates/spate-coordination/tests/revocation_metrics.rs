...
use spate_core::coordination::{CoordinationEvent, SplitCoordinator, SplitProgress};
...
fn a_real_revocation_moves_every_metric_seam() {
    ...
    assert!(
        histogram_count(&text, "spate_coordination_drain_duration_seconds")
            .is_some_and(|c| c > 0.0),
    ...
    assert!(
        saw_draining,
        "spate_coordination_splits_draining never read 1 while a drain was in flight"
    );
}
