...
//!
//! There is no I/O anywhere in this crate, so — unlike `S3Source` — the
//! constructors take no runtime handle and nothing here depends on `tokio`.
//! The shape to compare against is `KafkaSource`: `new` from a validated
//! config, `from_component_config` from the pipeline's opaque section.

use crate::config::DatagenSourceConfig;
use crate::encode::Encoder;
use crate::lane::{DatagenLane, LaneParts, Shared};
...
impl DatagenSource {
    ...
    /// string as [`EVENT_SCHEMA_JSON`](crate::EVENT_SCHEMA_JSON), reachable
    /// from the type an assembly already has in hand.
    #[must_use]
...
impl Source for DatagenSource {
    ...
    fn open(&mut self, ctx: SourceCtx) -> Result<(), SourceError> {
        ...
        self.state = Some(OpenState {
            issuer: ctx.issuer,
            shared: Arc::new(Shared::new(partitions)),
    ...
    fn poll_events(&mut self, timeout: Duration) -> Result<SourceEvent<DatagenLane>, SourceError> {
        ...
        let state = self.open_state()?;
        // Every gauge in this crate is written here and only here: one live
        // owner per series, on the controller thread (INV-10).
        if let Some(metrics) = &state.metrics {
            let remaining = if bounded {
                ...
                // An unbounded stream has no remainder. Publishing a budget
                // sentinel here would read as a real figure.
                0
            };
            let open = state
                .shared
        ...
        // `CommitReady` is declined. It is a latency optimization for a source
        // with a unit of work to finalize — a coordinated split whose slot
        // only frees once its watermark reaches the store — and a generator
        // has none: nothing downstream of a commit here is waiting on it.
        park(timeout);
        Ok(SourceEvent::Idle)
    }
...
mod tests {
    ...
    fn drained_arrives_only_after_every_lane_is_polled_past_its_last_batch() {
        ...
        // Now the poll that proves the last batch was consumed.
        for lane in &mut lanes {
    ...
    /// Watermarks go into a map and nowhere else. Stating it as a test is what
    /// keeps the docs' no-resumability claim checkable.
    #[test]
