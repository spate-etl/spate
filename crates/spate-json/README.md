...
JSON deserialization for the [Spate](https://github.com/spate-etl/spate)
framework. Decodes JSON payloads from a source into your own serde types or
dynamically-typed values.

Applications should depend on the **`spate`** facade crate with the **`json`**
...
source that never holds a whole object in RAM (`spate-s3`, an HTTP body), the
...
`spate_json_deser_records_dropped_total{reason}`, and continues; `on_error: fail`
...
is compiled. It is off by default. `simd-json` is a different parser, so
decoding is not byte-for-byte identical on every input, and the `serde_json`
knobs above do not apply under it; the JSON connector guide's Backends section
gives the differences. `from_reader` is never used — decoding always operates on
the in-memory payload slice.
