...
use spate_test::{BytesPassthrough, PipelineRun, capture_sink, memory_source, wait_until};
use std::sync::Arc;
...
struct BrokenFinishEncoder {
    /// Rows accepted, across every clone.
...
fn a_finalize_failure_on_the_shutdown_drain_fails_the_run() {
    ...
    let encoder = BrokenFinishEncoder::default();
    let encoded = Arc::clone(&encoder.encoded);
    ...
    // Buffered well short of the 30s commit tick, so only the shutdown drain
    // finalizes it.
    wait_until(Duration::from_secs(10), "the row is buffered", || {
