...
fn run_instance(
    ...
    // Demo-fast takeover; production floors are far higher (see the
    // deployment guide). The store was built from the same constant, and
    // the coordinator rejects a store whose TTL diverges from
    // lease_duration.
...
/// Drives the drift check directly, over both of its answers: the run below
/// reaches neither reliably.
///
/// The driver calls [`SplitSource::validate_resume`] only for a gain that
/// carries committed progress. A split here is read well inside one
