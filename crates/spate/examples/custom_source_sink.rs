...
///
/// Why not the meta-only `ShardRouter` (which `KeyHashRouter` implements):
/// the chain below fans one upload out into several order lines, and every
/// child of a `flat_map` inherits its parent's `RecordMeta` — that shared
/// metadata is what lets one parent ack resolve across all its children.
/// A meta-only router sees only that metadata, so it would put every child
/// of one upload on the same shard no matter which customer each order
/// belongs to. Routing on the payload is the only way to place them
/// independently.
///
/// The router must be **deterministic across retries**, and that is a
/// correctness requirement rather than a nicety: delivery is at-least-once,
/// so a record can be replayed after a failure, and a router that answered
/// differently the second time would write the same order into two shards —
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut shards_of_upload: BTreeMap<i64, BTreeSet<usize>> = BTreeMap::new();
    let mut rows = 0;
    for (endpoint, lines) in written.iter() {
        ...
        for line in lines {
            let (customer_id, order_id) =
                parse_order_line(line).expect("every row is one encoded order line");
            rows += 1;
            // Same customer, same shard, always — the determinism the
            // comment on `ByCustomer` calls a correctness requirement.
            ...
            shards_of_upload
                .entry(order_id / LINES_PER_BATCH)
                .or_default()
                .insert(shard);
        }
    }
    assert_eq!(rows, 2 * per_partition * LINES_PER_BATCH);
    ...
    // And the payoff: children of one upload sit on different shards, which
    // a meta-only router could not have produced — they share their
    // parent's metadata and nothing else.
    assert!(
