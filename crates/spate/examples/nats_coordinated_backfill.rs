...
//! cooperatively — it stops intake at a record boundary and commits its
//! tail before handing them back, so the move replays nothing — and the
//! second claims them. Each instance prints the objects it covered.
//!
...
//! key dropped. Nothing has to expire. The survivor sees the released
//! records on its watch and picks them up as soon as it holds the
//! leadership that assigns them — seconds after the signal rather than a
//! lease after it — and because the departing instance commits its tail
...
//! last heartbeat; heartbeats run every lease/3, so that lands between two
//! thirds of a lease and a full one after the death. The expiry reaches the
//! survivor as a limit marker, and the leader then withholds the dead
//! instance's splits for `rebalance_delay` — the window that lets a
...
///
/// The id alphabet is `[A-Za-z0-9_-]`, so a hostname's dots are rewritten
/// rather than rejected at startup.
fn instance_id() -> String {
    let raw = std::env::var("POD_NAME")
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let tuning = CoordinationConfig {
        ...
        // costs a slow rebalance, never a race. Left at the default the
        // deadline expires first and every handoff is forced instead of
        // cooperative — which is safe (the tail replays) but is not what
        // this example is here to show.
    ...
    // side by side: the object lists are disjoint and together they are the
    // whole prefix.
