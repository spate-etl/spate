...
//!    replays instead of consuming forever while committing nothing.
...
/// Three storefront payloads, one row each. Far under every sealing
/// threshold, so the linger below is what seals them, and few enough that
/// every write the sink was asked to make can be read back one by one.
const ORDERS: [&str; 3] = [
    "order_placed:1001",
...
"#;
    ...
    // Replica choice is a per-shard round-robin cursor shared by every
    // write in flight on that shard, so with the default two the replica a
    // retry lands on depends on what a sibling write picked in between. One
    // in flight makes the rotation below this batch's own.
...
"#;
    ...
    // A watermark is one-past-last-committed, so it sitting at or below the
    // first offset of the abandoned wave is the same statement as every
    // record of that wave replaying.
    assert!(
