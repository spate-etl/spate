//! A realistic pipeline that needs nothing installed — and the only example
//! that ends the way production does.
//!
...
/// Two shards per sink, so "an order and its payment share a shard" is a
/// claim with something to check rather than a tautology over one shard.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // it stops — with pending work abandoned rather than written, which can
    // strand a payment whose order never landed. The final watermarks sum to
    // the configured budget exactly when the source exhausted it instead.
    ...
    assert!(
        ...
        "the generated mix is ~60% orders, ~35% payments, ~5% refunds"
    );
