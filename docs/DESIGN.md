...
| Source coordination model | dynamic work-stealing splits: a leader-elected worker runs the source's planner; workers lease splits toward `min(max_in_flight, fair share)`, heartbeat at lease/3, steal pairwise (victim > mine + 1) when nothing is unclaimed | static per-source partitioning cannot rescale mid-job and bounds skew by luck; fine-grained planner-sized splits + stealing attack skew twice; the pairwise rule converges to ±1 at stable load, and (unlike a fair-share gate) lets a worker holding nothing steal into an at-equilibrium fleet. Two measured qualifications: completions near the end of a bounded job re-arm the rule, so a nearly-done split can move more than once (bounded churn — total moves bounded by the number of completions, each hop replaying at most one commit interval); and a newcomer only shares once a steal round and a committed watermark line up, so a job that completes inside that window ends on one instance — for a short job the optimal outcome (zero replay), not starvation |
...
| Embedded consensus (Raft) | still rejected | no worker framework embeds consensus for work distribution; a voter set fights autoscaling (stable membership, per-node state); the problem needs linearizability only at the per-split commit — exactly what one CAS provides |
...
| Steal eligibility | only a split with a committed watermark is movable; imbalance is still measured on the owner's total load | an uncommitted split has no resume point — moving it replays it in full, not the one commit interval a steal is meant to cost; waiting for the first checkpoint restores the documented replay bound, at the price of a freshly-claimed plan rebalancing one checkpoint tick later. Measured: eliminates whole-split replays at every scale and leaves long-job sharing untouched. Owner death is unaffected (that path is an expiry claim, not a steal) |
| Cooperative handoff | live-owner transfers are consent-first: an under-target worker writes an ephemeral `handoff.{victim}` request instead of CASing the lease; the victim drains up to `handoff_max_grants` hash-picked splits (intake stopped at a safe boundary, emitted tail acked, final watermark committed `completed: false`) and releases them; the requester claims each as `released` with a resume point covering everything the victim emitted; a request unanswered for `handoff_rounds` heartbeat rounds falls back to the CAS steal | fencing a live owner mid-read costs up to a commit interval of duplicates per move, and endgame churn multiplies that per hop — a granted handoff is replay-free and ping-pong costs latency, never rows. Requests skip the committed-watermark gate (the drain manufactures the resume point a steal must wait for) while the fallback steal keeps it, so the worst case degrades exactly to the steal bound; the request key is liveness-only (the record CAS stays the sole fence); a dead owner still loses the split within ~one lease (expiry claim), and a wedged owner loses its committed splits to the fallback steal within the round budget — its never-committed splits wait for its first commit, exactly as in the steal era. Precedent: Kafka incremental cooperative rebalancing (KIP-429), Flink stop-with-savepoint |
