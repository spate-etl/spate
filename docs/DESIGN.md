...
| Source coordination model | dynamic work-stealing splits: a leader-elected worker runs the source's planner; workers lease splits toward `min(max_in_flight, fair share)`, heartbeat at lease/3, steal pairwise (victim > mine + 1) when nothing is unclaimed | static per-source partitioning cannot rescale mid-job and bounds skew by luck; fine-grained planner-sized splits + stealing attack skew twice; the pairwise rule provably converges to ±1 and cannot oscillate, and (unlike a fair-share gate) cannot starve newcomers |
