...
# `0` here would be indistinguishable from "caught up".
...
| `spate_sink_abandoned_batches_total` | counter | `shard` | Batches whose acknowledgments failed, so the data replays after restart — a fatal error class, exhausted `retry.max_attempts`, a panicking write task, or a drain deadline expiring at shutdown. Only the last is a shutdown event; see [What a flush contains](#what-a-flush-contains). |
...
| `spate_coordination_split_failures_total` | counter | | Poison reports for a split: an explicit `fail` from the source, and a gain whose carried progress the source refused on resume. Counted when the report is offered rather than when it lands, and a refused report is re-offered until it does, so a split can count more than once per tenancy. |
...
`drain_duration_seconds` and `assignment_latency_seconds` are separate
families. One family split by a `phase` label would assert a containment
that does not hold, so do not add, subtract, or otherwise combine them.
