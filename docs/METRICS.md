...
| `etl_sink_inflight_batches` | gauge | `shard` | Sealed batches not yet settled — those being written **plus** any sealed batch still queueing for one of the shard's `inflight.max_per_shard` slots. It can therefore sit one above the cap while a batch waits, and further above it in the pathological case where a single chunk crosses `batch.max_bytes` on its own (see `etl_sink_permit_wait_duration_seconds`). Read it against the cap for saturation, not as an equality. |
