...
twenty pods is a group of a hundred and sixty. Consumer-group rebalances get
slower and more disruptive as the group grows.
...
- Bad, because the rebalance choreography is ours to maintain, and each step
  above is required for correctness in a way that is not apparent from the code.
- Bad, because zero-timeout polling busy-spins, so idle lanes must block briefly
  on their queue.
...
- **The choreography above is the 2026-07-05 understanding, not a current
  spec.** The implementation has since moved to deferred completion, in which
  the revocation spans two `poll_events` calls and finishes at `unassign()`, and
  a message that reaches the main queue is rewound with `seek` rather than
  merely routed. The living specification is the module documentation on
