...
tuning preference but what makes the framework's delivery promise true.
...
requirement is one that has to have been read.
...
- Bad, because an operator who wants weaker durability (a best-effort
  side-channel topic) cannot have it through this sink, and there is no escape
  hatch by design.
