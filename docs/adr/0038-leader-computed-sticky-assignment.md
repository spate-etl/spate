...
Centralizing costs nothing in safety, because an assignment **is not a fence**.
A stale, split-brained or wrong leader can produce bad balance but never two
owners, because the durable record's compare-and-set still decides ownership
...
centralized in an elected leader. The designs that were peer-to-peer have since
moved to one, having accumulated the same symptoms (an unraisable steal cap and
an uncorrected disbalance). Arriving independently at a design those have
abandoned is a signal worth acting on.
