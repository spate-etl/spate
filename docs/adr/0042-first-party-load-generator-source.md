...
routing, joins and aggregation. Those need records that *reference each
...
scope. That is a small programming language, which is a different product with a
...
- Neutral, because the generator keeps no durable progress. A restart
  ...
  at-least-once source. It is safe under INV-1, and stated in the crate docs,
  the README and a startup `WARN` so that nobody mistakes it for a resumable
