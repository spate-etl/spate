...
The two multiples are the point of the CPU rig: the borrowed decoder is ~7.4×
faster to decode and ~4.3× faster through the whole in-process pipeline (decode →
...
borrowed `serde_avro_fast` lands **+0.12%** and owned `serde_avro_fast`
**+0.08%** from owned `apache-avro` — every arm sits within **±0.3%** across n=3
reps (medians ~213.5k rows/s). No measurable difference, because the ClickHouse
sink, not the decoder, is the limiter. The insert format does not move the rate
either: `fast_borrowed + RowBinary` lands within **−0.26%** of the same decoder on
