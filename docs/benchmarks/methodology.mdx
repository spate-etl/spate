...
# Cross-format decode → deser-formats.jsonl (no broker, no server; same data as
# Avro and JSON). One invocation = one arm. Two workloads (SHAPE=order|batch), two
# record modes (RECORD=typed|value); the batch runs Avro's nested datum against
# JSON's single/array/ndjson framings — 12 arms, n = 9 reps each.
for R in typed value; do
  for F in avro json; do
    SHAPE=order FORMAT=$F RECORD=$R REPS=9 \
      RESULTS=benchmarks/results/deser-formats.jsonl \
      cargo run -p benchmarks --release --bin deser_formats
  done
  SHAPE=batch FORMAT=avro RECORD=$R REPS=9 \
    RESULTS=benchmarks/results/deser-formats.jsonl \
    cargo run -p benchmarks --release --bin deser_formats
  for FR in single array ndjson; do
    SHAPE=batch FORMAT=json FRAMING=$FR RECORD=$R REPS=9 \
      RESULTS=benchmarks/results/deser-formats.jsonl \
      cargo run -p benchmarks --release --bin deser_formats
  done
done
...
# 16 partitions × 4 threads × 10 µs/record × 256 B. The binary takes a
...
# 2 reps per mode. `consume` is time-boxed by DURATION_S (MESSAGES only sizes the
# fill), and exits 3 if it drained the topic — a drain measures how fast a topic
# empties, not a throughput ceiling, so treat a 3 as "produce more and rerun".
PARTITIONS=16 THREADS=4 PAYLOAD=256 MESSAGES=10000000 \
  cargo run -p benchmarks --release --bin kafka_topology produce
for MODE in perthread split; do for _ in 1 2; do \
  MODE=$MODE PARTITIONS=16 THREADS=4 WORK_US=10 PAYLOAD=256 DURATION_S=30 \
    RESULTS=benchmarks/results/kafka-topology-confirm.jsonl \
    cargo run -p benchmarks --release --bin kafka_topology consume; \
done; done

# Kafka source ceiling → kafka-source-ceiling.jsonl (raw client, no framework)
# The prefetch × threads sweep behind the tuning guidance, on a Redpanda broker
# (BROKER defaults to redpanda; it is recorded as a variant key, so Redpanda and
# Apache Kafka arms never aggregate). WORK_US=0 so the fetch path is the only
# thing measured; QUEUED_MIN_MESSAGES=0 means "set nothing", the shipped default.
#
# BOTH payload sizes are swept, and the payload loop is INSIDE the rep loop so
# the two sizes interleave — the cross-size comparison is the page's headline
# claim, and running one size then the other is exactly the sequential A/B that
# has faked a 30% delta from thermal drift on this box.
#
# The backlog must outlast a saturated window: `consume` exits 3 and emits
# nothing if it drains the topic (a drain measures how fast a topic empties,
# not a ceiling), and exits 4 if messages leaked to a main queue.
for b in 64 256; do
  PARTITIONS=16 PAYLOAD=$b MESSAGES=60000000 \
    cargo run -p benchmarks --release --bin kafka_topology produce
done
for rep in 1 2 3; do
  for q in 1000 3000 10000 30000 100000 300000; do for b in 64 256; do \
    MODE=split INSTANCES=1 PARTITIONS=16 WORK_US=0 DURATION_S=6 \
      THREADS=1 PAYLOAD=$b QUEUED_MIN_MESSAGES=$q \
      RESULTS=benchmarks/results/kafka-source-ceiling.jsonl \
      cargo run -p benchmarks --release --bin kafka_topology consume; \
  done; done
  for t in 1 2 4 8; do for b in 64 256; do \
    MODE=split INSTANCES=1 PARTITIONS=16 WORK_US=0 DURATION_S=6 \
      THREADS=$t PAYLOAD=$b QUEUED_MIN_MESSAGES=100000 \
      RESULTS=benchmarks/results/kafka-source-ceiling.jsonl \
      cargo run -p benchmarks --release --bin kafka_topology consume; \
  done; done
done
# Where the byte cap binds, 64 B only: flat from 300000 up, because
# queued.max.messages.kbytes (65536 x 1000 = 65.5 MB) caps at ~1.02M messages.
for rep in 1 2 3; do for q in 1000000 3000000; do \
  MODE=split INSTANCES=1 PARTITIONS=16 WORK_US=0 DURATION_S=6 \
    THREADS=1 PAYLOAD=64 QUEUED_MIN_MESSAGES=$q \
    RESULTS=benchmarks/results/kafka-source-ceiling.jsonl \
    cargo run -p benchmarks --release --bin kafka_topology consume; \
done; done
...
`etl_sink_records_total` delta / window (durable acks). The broker is capped at
...
(`client`/`broker`, from the raw baseline and the sink's `etl_kafka_*` broker
