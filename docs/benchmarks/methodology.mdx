...
- **Deterministic counters gate every pull request.** Allocation counts on the
  ...
  and the planning an object-store backfill does per plan. New benches are
  written as one workload shape parameterised over the axis that changes its
  cost — a listing profile, a codec — rather than one case per function,
  because attribution below the shape comes from the callgrind profile the
  run already writes and a finer set of cases would only duplicate it. Where a
  bench generates its own corpus, that corpus is produced from a fixed
  sequence and pinned by a test
