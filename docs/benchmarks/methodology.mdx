...
- **Deterministic counters gate every pull request.** Allocation counts on the
  ...
  and the planning an object-store backfill does per plan. Each is one
  workload shape parameterised over the axis that changes its cost — a
  listing profile, a codec — rather than one case per function: below the
  shape, attribution comes from the callgrind profile the run already writes,
  which a finer set of cases would only duplicate. The corpora are generated
  from a fixed sequence and asserted reproducible
