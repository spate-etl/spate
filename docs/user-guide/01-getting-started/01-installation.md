...
pipelines against (see [Testing pipelines](../03-guides/testing-pipelines.md)
and the [memory connector](../04-connectors/memory.md)).
...
> `apache-avro`). None of their types appear in `etl`'s public trait bounds,
...
> is documented as exempt from etl-rs's stability promises.
...
- The `kafka` feature builds `rdkafka`, which compiles librdkafka from
  ...
  takes a few minutes. The [Docker guide](../05-deployment/docker.md)
...
Then head to the [Quickstart](02-quickstart.md) — it runs a full pipeline
