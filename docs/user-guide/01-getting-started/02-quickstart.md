...
in `main`) — see [Installation](01-installation.md).
...
sections — see [Configuring pipelines](../03-guides/configuring-pipelines.md)
and the [configuration reference](../07-reference/configuration.md).
...
are identical — see the [memory connector](../04-connectors/memory.md).
...
- Everything inside the closure is statically composed — `filter` and `map`
  ...
  chain boundary. See [Architecture](../02-concepts/01-architecture.md).
- `.into_runtime(source)` instead of `.run(source)` hands you the runtime
  *before* it starts, so you can grab a `ShutdownHandle` and spawn the run —
  exactly the pattern for [testing pipelines](../03-guides/testing-pipelines.md).
...
commit. [Delivery guarantees](../02-concepts/02-delivery-guarantees.md)
...
- Swap the mocks for real connectors:
  [Your first pipeline](03-first-pipeline.md).
- Understand what each builder step desugars to:
  [Assembling a pipeline](../03-guides/assembling-a-pipeline.md) and
