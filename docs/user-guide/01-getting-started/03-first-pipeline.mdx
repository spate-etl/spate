...
SIGTERM. It mirrors `crates/spate/examples/kafka_avro_to_clickhouse.rs` and
its YAML (`crates/spate/examples/kafka_avro_to_clickhouse.yaml`); every
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let report = pipeline
        .sink(sink)?
        .chains(move |ctx| {
            // The sink's chunking (per-sink YAML `chunk:`, or the default),
            // bound before `with_metrics` moves `ctx.pipeline`.
