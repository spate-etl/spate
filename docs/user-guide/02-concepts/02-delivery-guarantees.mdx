...
[ClickHouse connector](../04-connectors/sinks/clickhouse/README.mdx). For the
Kafka sink, a batch counts as durable when **every** message's delivery
report has confirmed under `acks=all` — the write returns only then, so a
single failed or timed-out report stalls the watermark and the batch
replays. See the [Kafka sink](../04-connectors/sinks/kafka/README.mdx).
...
choreography are in the [Kafka connector](../04-connectors/sources/kafka/README.mdx) and
