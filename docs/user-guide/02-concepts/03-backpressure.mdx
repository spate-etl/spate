...
`spate_backpressure_paused`, `spate_backpressure_pause_events_total`
(a flapping indicator when high), and `spate_backpressure_inflight_bytes`
...
growing consumer lag (`sum(spate_source_lag_records)`), which is the correct,
