...
|---|---|---|
| **Retryable** | Transient sink I/O: a timeout, a connection reset, one replica down | Handled by the sink layer: the same sealed batch retries on the next healthy replica with capped exponential backoff and circuit breakers. Not your problem unless it persists — watch `etl_sink_retries_total` and `etl_sink_replica_healthy`, with `etl_sink_replica_errors_total` to pin the failing endpoint and `etl_sink_shard_healthy` for the whole-shard escalation. The retry counter only moves on an *attempt*, so pair it with `etl_sink_retry_backoff_seconds` — the gauge that says whether the shard is asleep between attempts on an available replica right now, and for how long. |
