...
That last one is not an error, and it is worth knowing why it has its own
series. A deserializer can report that it is **not ready** rather than that it
failed — waiting on a schema fetch, say. The framework retries it **without
pausing the source**, because the source is not the problem and pausing it
would slow a pipeline that is merely waiting on a lookup. A rising count is
...
[Monitoring](../05-deployment/monitoring.mdx) for the full alerting story.
