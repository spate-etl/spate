...
`crates/spate/examples/kafka_avro_to_clickhouse.rs` with its YAML next to it,
built under the `full` facade feature; the connector, encoder and chain
blocks below are rendered from it. The zero-infrastructure variant is
