...
The block below is the exception: it is a shape, not a program. `deserializer`
and `encoder` are used without ever being bound, so it does not compile — an
assembly with nothing left out is the example, and the steps that follow fill
each elision in with the code that does compile.
...
```rust file=crates/spate/examples/memory_pipeline.rs region=options
```
