...
The block below is the exception: it is a shape, not a program. `Order`,
`deserializer` and `encoder` are used without ever being bound, and the
connector imports are elided along with them, so it does not compile. The
