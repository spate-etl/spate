...
These top-level sections belong to the framework — `pipeline`, `admin`,
`checkpoint`, `backpressure`, and `metrics`:
...
feed Kubernetes ConfigMap/Secret values in. The forms (`${VAR}`,
