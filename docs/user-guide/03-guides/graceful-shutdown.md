...
description is [docs/DESIGN.md](../../DESIGN.md) § Errors, panics,
shutdown, health.
...
If the sink cannot flush everything by the deadline — a ClickHouse outage
mid-shutdown, say — the remaining batches are **abandoned loudly**: a log
line and a metric, their acknowledgements are failed, and their partitions'
...
[Assembling a pipeline](assembling-a-pipeline.md)). If an assembly
...
- [Docker](../05-deployment/docker.md) and
  [Monitoring](../05-deployment/monitoring.md) — probes, signals, and the
  metrics that surface abandons and stalled watermarks.
- [Backpressure](../02-concepts/03-backpressure.md) — why a slow sink shows
