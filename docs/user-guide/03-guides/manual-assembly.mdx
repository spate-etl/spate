...
running. The rustdoc on `spate::pipeline::Pipeline` carries the same mapping
...
`spate::sink` (`shard_queues`, `SinkPool`, `SinkBundle`, `SinkParts`), and
`spate::backpressure` (`InflightBudget`).

## Step by step

The sketch below mirrors what the builder's own implementation does
(`crates/spate-core/src/pipeline/builder.rs`); construction of your
...
use spate::pipeline::metrics_settings;
use spate::telemetry::{self, LogFormat};
...
let metrics = spate::metrics::install(&metrics_settings(&config))?;
...
```rust
use spate::metrics::{ComponentLabels, SinkShardMetrics};
use spate::sink::{SinkBundle, SinkPool, shard_queues};
...
```rust
use spate::pipeline::{PipelineRuntime, RuntimeOptions, SinkRuntime};
