...
`crates/spate-test/tests/bundle.rs` (a whole-assembly test) and
`crates/spate/examples/memory_pipeline.rs` (the same pattern as a runnable
demo).
...
the blocking `run` on a thread. From `crates/spate-test/tests/bundle.rs`:
...
last offset** — the "next position to read", the same convention the
framework commits everywhere:
