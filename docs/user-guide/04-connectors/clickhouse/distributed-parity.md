...
[Why direct-to-shard, not a Distributed table](./README.md#why-direct-to-shard-not-a-distributed-table)).
...
> sink placed on the wrong shard is simply invisible. This is documented
...
- The predicate is a literal `=` or `IN` (combined with `AND`/`OR`) over the
  ...
  parameters** (`{p:String}`) do **not** prune — the optimizer can't evaluate
  the sharding expression at planning time.
- The sharding key is deterministic (it is — `xxHash64` of a column).
...
> move. Nothing merges across shards: the old shard keeps its copies
...
> topology is the benign case: replayed rows land on the same shard, where
> dedup tokens and the collapse pattern apply as usual. See
> [Delivery guarantees](../../02-concepts/02-delivery-guarantees.md).

## Related

- [Sink sharding](../../02-concepts/05-sink-sharding.md) — the connector-
...
- [Backpressure](../../02-concepts/03-backpressure.md) — how one saturated
