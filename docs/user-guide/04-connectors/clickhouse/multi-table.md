...
[multi-sink split](../../02-concepts/06-multi-sink.md) concept: the split
...
[configuration reference](../../07-reference/configuration.md#sinks). Every key
documented for a single [ClickHouse sink](README.md#configuration) applies
per-entry.
...
[routing DSL walkthrough](../../02-concepts/06-multi-sink.md#the-routing-dsl).

## What carries over per table, unchanged

Each branch is a full ClickHouse sink, so everything on the
[ClickHouse sink page](README.md) holds **per table**:
...
- **Direct-to-shard writes** with `wait_end_of_query=1`; a table's write is its
  ...
  touched has acked (see [delivery](../../02-concepts/06-multi-sink.md#at-least-once-across-tables-is-free)).
- **Per-batch `insert_deduplication_token`** per table. The same
  [window warning](README.md#deduplication-tokens--and-the-window-warning)
...
- **Metrics per table**: each sink's `etl_sink_*` series carry the sink name as
  their `component` label, so `etl_sink_records_total{component="gauge"}` and
...
> record — inflating checkpoint lag and restart replay. Keep rare-table
> `linger` well under `checkpoint.stalled_fail_after`, and prefer sealing on a
> modest `max_rows` so `linger` only bites in genuine lulls. The full reasoning
> — including segregating the source by kind to dissolve the tension — is in the
> [concept page's trade-off section](../../02-concepts/06-multi-sink.md#the-trade-youre-making-part-size-vs-checkpoint-lag).

## `distributed_check` per table

Each table can carry its own [`distributed_check`](distributed-parity.md): the
...
- [Multi-sink split](../../02-concepts/06-multi-sink.md) — the concept, the routing DSL, and the at-least-once + part-size trade.
- [ClickHouse sink](README.md) — everything a single sink does, which each branch inherits.
