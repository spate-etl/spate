...
opt-in [fast backend](fast-backend.mdx)):
...
- `build_serde_fast::<T>()` / `build_fast::<F>()` (feature `avro-fast`) —
  ...
  [the fast backend](fast-backend.mdx).
...
trade-offs, and the license note: [the fast backend](fast-backend.mdx).

## Related

- [The fast backend](fast-backend.mdx) — opt-in single-pass decoding and
  zero-copy records.
- [Kafka source](../../sources/kafka/README.mdx) — the payloads this format decodes.
