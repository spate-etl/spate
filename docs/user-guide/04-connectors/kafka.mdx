...
| `statistics_interval` | duration | `5s` | librdkafka statistics emission interval, feeding the lag metrics and the `etl_kafka_source_*` families (broker health, latency, queue saturation, group stability); `0s` disables statistics and those families with it. |
...
TLS/SASL is off by default (the bundled librdkafka is built without it).
Enable the opt-in **`kafka-tls`** feature and set the security properties
through the same `rdkafka:` passthrough — they are not denied:
...
source:
  kafka:
    ...
    rdkafka:
      ...
      ssl.certificate.location: /etc/kafka/client.pem   # mTLS: client cert
      ssl.key.location: /etc/kafka/client.key
```

Setting any security property without the feature is rejected at config load
with an actionable message. The full property matrix (TLS, mTLS, SASL
SCRAM/PLAIN, secrets via `${VAR}`) and the identical sink configuration are in
