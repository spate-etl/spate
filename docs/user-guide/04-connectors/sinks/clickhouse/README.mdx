...
That is from `crates/spate/examples/kafka_avro_to_clickhouse.rs`, a complete
program that builds under the `full` facade feature. The single `sink:`
section is addressed as `"default"`; a named sink resolves under its own
name. Between this line and `pipeline.sink(sink)?` sits the optional
