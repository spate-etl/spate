...
| `batch` / `inflight` / `retry` / `breaker` | sections | framework defaults | Shared sink-pool tuning (batching, in-flight limit, retry backoff, circuit breaker). Keys and defaults are in the [configuration reference](../../../07-reference/configuration.mdx#sink-pool); the breaker is described under [Replica rotation](#replica-rotation-and-the-circuit-breaker) below. |
...
| `validate_schema` | string | `off` | Startup + first-record schema validation: `off`, `names`, `full`. See [Schema validation](../../../03-guides/schema-validation.mdx). |
...
check — see [Schema validation](../../../03-guides/schema-validation.mdx). The
