...
That is from `crates/spate/examples/kafka_avro_to_clickhouse.rs`, which runs
end to end under the `full` facade feature. The single `sink:` section is
addressed as `"default"`; a named sink resolves under its own name. Between
this line and `pipeline.sink(sink)?` sits the optional pre-flight check —
see [Schema validation](./schema-validation.mdx).
