...
any of the above. The row declares the field `T` takes for either format, for
any `T` that format writes on its own. Use them when you control the schema
and the function qualifies.
