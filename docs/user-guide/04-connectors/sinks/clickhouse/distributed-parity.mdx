...
a sharded Native pipeline that needs a Kafka topic of Avro batches and the
...
`SensorEvent` is the row the chain emits, and `ShardKey::Str` borrows its
`sensor` field, so extraction allocates nothing on the per-record path.
...
carries — `Owned<SensorEvent>` for an owned row struct, matching the family the
