...
The key extractor is a plain `fn` item, not a closure — it is higher-ranked
over the payload lifetime — and it returns a `ShardKey`. The router owns the
...
Native pipeline that runs with
