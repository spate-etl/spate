...
takes. In `crates/spate/examples/kafka_avro_flatmap_clickhouse.rs`, which runs
with
`cargo run --release -p spate --features full --example kafka_avro_flatmap_clickhouse`,
a nested batch is decoded and fanned out into one such row per reading:
...
The encoder is built from the schema the sink fetched:
