...
takes. `crates/spate/examples/kafka_avro_flatmap_clickhouse.rs` decodes a
nested batch and fans it out into one such row per reading; it needs a Kafka
topic of Avro batches and the target table, and runs with
