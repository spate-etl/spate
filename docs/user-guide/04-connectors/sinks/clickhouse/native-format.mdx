...
`AggregateFunction` is unsupported by design in *both* formats, because its
state is opaque and version-dependent. Feed those tables with raw rows through a Null
