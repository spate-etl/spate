...
let encoder = spate::clickhouse::NativeEncoder::<Owned<MyRow>>::new(native_schema);
...
Native blocks are larger per unit of work, set a larger terminal-stage chunk
target (the sink's YAML
[`chunk.target_bytes`](../../../07-reference/configuration.mdx#chunk), e.g.
512 KiB) so each block is substantial; the sink still merges blocks up to
