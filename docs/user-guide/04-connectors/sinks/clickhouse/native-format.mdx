...
decodes a nested batch and fans it out into one such row per reading; it needs
a Kafka topic of Avro batches and the target table, and runs with
...
`SensorEvent` is the row the terminal stage encodes. Its field order is the
...
struct is wrapped as `Owned<SensorEvent>` (a zero-copy borrowed row uses its
