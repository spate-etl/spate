---
description: "ClickHouse sink performance tuning covers batch size, writer parallelism, I/O threads, compression, and async_insert as starting points to measure from."
---
...
Some insert settings are fixed no matter how you tune. Three are the
at-least-once contract: the sink always sends `insert_deduplicate=1`, a
per-batch `insert_deduplication_token`, and `wait_end_of_query=1`. Two more
are the wire format's: `input_format_with_names_use_header=1` and
`input_format_with_types_use_header=1`, which are what make the server check
the insert header against the table. All five are **reserved** — they cannot
be overridden through the `settings:` map, and the config rejects them at
load. Together they mean a successful insert is
