...
ceiling on that tracking: a partition at the ceiling stops being polled until
acknowledgments retire batches. Under very high insert rates — especially many
small parts — a partition can sit at the ceiling before ClickHouse is
...
(replicated tables keep a window by default) so a retried batch is actually
deduplicated:
