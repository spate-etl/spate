...
`windows` counts the statistics windows the partition has gone without
fetching, so at the default `statistics_interval` each one is 5 seconds. The
state names the cause. `stopped` or `none` is a partition this process stopped
fetching, and `offset-query` or `offset-wait` is a leader or an offset lookup
that has not answered; librdkafka retries a failed lookup by moving the
partition between those two, so an episode of one cause reports both. An `INFO`
line closes the episode when the partition resumes, so the log says whether it
is still going.
