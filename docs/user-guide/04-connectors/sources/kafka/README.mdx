...
| `spate_kafka_source_partition_fetching` | gauge | `partition` ⚠ | 1 while librdkafka is fetching the partition, 0 in every other fetch state, and published only for the partitions this member holds. The state name goes to a log line rather than a label. Backpressure does not move the gauge, since librdkafka counts a paused partition as fetching. |
...
- `spate_kafka_source_partition_fetching == 0` — the member holds the partition
  and is not consuming it. The pipeline then goes quiet with a full assignment,
  a healthy group and no error anywhere. Give this one a `for:` window of a
  minute or so, since a partition reads 0 while librdkafka resolves its offset
  after an assignment.
