...
`per_partition_detail` is on. The line is emitted once per episode rather than
once per statistics window, and names the librdkafka fetch state and the
partition's offsets.
...
fetching, and `offset-query` is a leader or an offset lookup that has not
answered. An `INFO` line closes the episode when the partition resumes, so
the log says whether it is still going.
