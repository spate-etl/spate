...
- **Cardinality is mostly opt-in**: `partition`-labelled series appear only
  ...
  `etl_source_lag_records`, which is always per partition — consumer lag has
...
- **A shard asleep between retries** — `etl_sink_retry_backoff_seconds`
  ...
  waiting for a probe window instead, and `etl_sink_shard_healthy == 0` is the
  signal for that.
- **A slow sink** — the p99 of
  `etl_sink_write_duration_seconds{outcome="ok"}` above your write budget.
  Reach for this one, not `etl_sink_flush_duration_seconds`: flush spans seal
  ...
  (`etl_sink_permit_wait_duration_seconds`), backoff says *asleep between
  attempts* (`etl_sink_retry_backoff_seconds`), shard health says *asleep
  waiting for a probe window* (`etl_sink_shard_healthy == 0`, where the backoff
  gauge reads `0`), write says *slow* — and `etl_sink_flush_duration_seconds`
...
`etl_sink_replica_healthy`, `etl_sink_shard_healthy` (0 = no circuit-closed
replica left in the shard — the whole-shard outage escalation, with
`etl_sink_replica_errors_total` attributing the failures per endpoint), and
`etl_queue_full_events_total` series described in
