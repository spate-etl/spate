...
not a free default. The queue bounds the latency of whatever sits in it, so a
deeper queue means a chunk waits longer before a shard worker ever sees it. The
default holds the best flush p99 by an order of magnitude; a deep queue can be
worth tens of percent of throughput on a saturated pipeline.

The curve is **not** smooth, so choose an end of it rather than a middle: the
first step up from the default costs nearly all of the latency for almost none
of the throughput, while the deepest settings recover the throughput for
comparatively little more. Stay at the default for latency-sensitive pipelines;
if throughput dominates, go well past the next step up rather than nudging.

:::note[A full queue does not always mean the sink is slow]
The queue-full trigger is **rate-driven as well as capacity-driven**. It fires
whenever chunks arrive faster than a shard worker can service them — which can
happen while the sink itself has plenty of headroom, purely because the chain
is producing many small chunks. Measured against a null writer that completes
instantly (no sink slowness by construction), a fast chain still drove hundreds
of pause events at the default chunk target, and raising
`ChunkConfig::target_bytes` removed them entirely.

The operational consequence: **a high `etl_backpressure_pause_events_total` is
not by itself a misconfiguration.** In the measurements behind
[Chunk sizing](../../benchmarks/chunk-sizing.mdx) the *best-latency*
configuration also had the *most* pauses. Read pause counts together with
`etl_sink_flush_duration_seconds` and sink throughput before tuning against
them — and if pauses are high while the sink is demonstrably idle, reach for
chunk size, not queue capacity.
:::
