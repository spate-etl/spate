...
Only `ci` retries; `docker` does not. A fixture that boots a container waits for
it to answer a real query, and on timeout reports that container's liveness,
exit code and log tail — so a container failure carries its own diagnosis.
...
from the fast suite, `Container test results` from the container job. For the
fast suite that is what keeps its `retries = 1` from hiding a flake behind a
green run; for the container job, which does not retry, it is what puts a
failure somewhere other than scrolled-past log text.
