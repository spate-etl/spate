...
branches and cache references under callgrind. The comparison is against the
previous run's counters, carried between runs by an `actions/cache` entry whose
exact key never hits and whose `restore-keys:` prefix therefore falls through to
the newest one — the same mechanism as the scheduled criterion baseline, and
restored **before** the benches run for the same reason.
