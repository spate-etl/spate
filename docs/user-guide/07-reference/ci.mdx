...
It is the one job on this path that builds in the release profile, because the
bounds are calibrated against optimised code — and it pays for that by sharing
the `release` cache entry the scheduled release-profile jobs populate from
`main`.
...
one its field paths were written against. A merge base that predates a bench
reports that bench as *new* rather than failing, and a merge base with no
benches at all — the first pull request after a bench lands — is reported as
having no baseline. Push and merge-group runs report absolute counts.
...
four path groups — `crates/spate-s3/`, `crates/spate-core/src/ops/`,
`crates/spate-avro/`, and the rigs under `benchmarks/` — plus the `ci: bench`
label. Committed results under
`benchmarks/results/` are chart data rather than code and select nothing, which
is a property of where that arm sits in the classifier's `case` rather than a
