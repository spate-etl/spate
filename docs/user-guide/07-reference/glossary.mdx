...
most-loaded owner — only when the victim holds at least two more than the
thief, so every transfer strictly narrows the gap and ownership converges
to ±1 without oscillating. The victim keeps working until its next write
fences, bounding the duplicate window to one commit interval.
