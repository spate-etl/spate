...
worker below its share with nothing unclaimed takes one split from the
most-loaded owner — only when the victim holds at least two more than
the thief (every transfer strictly narrows the gap, converging toward ±1
balance), and only a split that has already committed, so the thief has
a resume point. The victim keeps working until its next write fences,
bounding each move's replay to one commit interval.
