...
# only reaches unpublished members, and these are published. Generating both
# artifacts through this script is what keeps the two in agreement.
#
...
# The first-party set: the publishable workspace members, by directory name. A
# crate's directory under crates/ is its package name, and `bench/` sits
# outside as the one unpublished member. Read off the tree rather than through
# `cargo metadata` so the drift gate needs no toolchain beyond cargo-about.
first_party=""
for dir in crates/*/; do
    [ -d "$dir" ] || continue
    dir=${dir#crates/}
    first_party="$first_party ${dir%/}"
done
[ -n "${first_party// /}" ] || fail "no crates under crates/, so the first-party filter is blind"
...
if [ "$mode" = html ]; then
    ...
    awk -v fp="$first_party" '
        ...
        # THIRD-PARTY.md counts it once, so the page counts it once too.
        NR == FNR {
            if ($0 ~ /^<!-- BEGIN-LICENSE /) { block++; id = $0; sub(/^<!-- BEGIN-LICENSE /, "", id); sub(/ -->$/, "", id) }
            else if ($0 ~ /<code data-crate="/ && !firstparty[crate_of($0)]) {
                keep[block]++
        ...
        END {
            ...
            if (toc < 1) { print "attribution.sh: no TOC row was rewritten; the template lost data-license-id" > "/dev/stderr"; exit 1 }
...
# the next run: the summary must name exactly the license ids the filtered
# table holds, and sorting must lose no row.
ids_in_rows=$(printf '%s\n' "$rows" | cut -d"$bt" -f4 | LC_ALL=C sort -u)
ids_in_summary=$(printf '%s\n' "$summary" | cut -d"$bt" -f2 | LC_ALL=C sort)
[ "$ids_in_rows" = "$ids_in_summary" ] || fail "license ids drifted between the table and the summary"

crates_before=$(printf '%s\n' "$rows_third" | LC_ALL=C sort -u | wc -l | tr -d ' ')
