...
container_suites_for() {
    case "$1" in
        ...
        # behind the `datagen` feature, and as an unconditional dev-dependency
        # so the examples need no `required-features`. Either edge is enough to
        # break the facade's container suite.
