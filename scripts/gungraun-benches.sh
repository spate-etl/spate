...
#   ci.yml, both legs          --run [PKG...]
#   ci.yml, changes job        --pkgs-json
#
...
# bench failed, and 2 when nothing was discovered. The three are distinct
# because the callers act differently on each: a failure means wipe the
# partial measurement, while an empty tree is the ordinary state of an old
# merge base. `GUNGRAUN_*` variables are inherited by the cargo child, which
# is how the base leg passes `GUNGRAUN_SAVE_BASELINE`.
set -euo pipefail
...
# the same order on every machine (glob order is locale- and filesystem-
# dependent, and the report reads better when the tables do not shuffle).
discover() {
...
self_test() {
    ...
    # A discovery function that silently stopped matching would pass every
    # assertion above by making zero of them. The workspace has had gungraun
    # benches since they were introduced; zero means the glob is broken.
    [[ "$checked" -gt 0 ]] || fail \
...
run() {
    ...
    while read -r pkg bench; do
        ...
        if (set -x; cargo bench -p "$pkg" --locked --bench "$bench"); then
            ran=$((ran + 1))
        else
...
case "${1:-}" in
    ...
    --run) shift; run "$@" ;;
    "") discover ;;
    *) fail "unknown argument '$1' (expected --run, --pkgs-json, --self-test, or none)" ;;
esac
