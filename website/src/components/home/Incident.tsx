...
// The queue is full and the source lanes are paused at this instant.
const T_STILL = 5.35;
...
/** Evaluate the Spate sequence at its sink-stall frame. */
function build(): Piece[] {
  ...
  /** Place a record on its path at the still-frame time. */
  ...
  const spateLoop = (): Piece[] => {
    ...
    out.push(
      tracked('text', {x: 180, y: 82, textAnchor: 'middle', className: 'incident__fw-t'}, {
        opacity: [
          [0, 0],
          ...pauses.flatMap(
            ([a, b]) =>
      ...
      tracked('text', {x: 334, y: 158, textAnchor: 'middle'}, {opacity: fade(STALL[0], STALL[1])}, 'stalled'),
    );
    // the skipped record is counted
    out.push(
      tracked('text', {x: 180, y: 148, textAnchor: 'middle', className: 'incident__fw-t'}, {
        opacity: [
          [0, 0],
          [bad.fnT + 0.4, 0],
          [bad.fnT + 0.6, 1],
    ...
    out.push(
      ...
      tracked('text', {x: 180, y: 226, textAnchor: 'middle', className: 'incident__fw-t'}, {
        opacity: [
          [0, 0],
          [exit - 0.01, 0],
          [exit, 1],
    ...
    <div className="incident">
      <figure className="incident__panel">
        <svg viewBox={`0 0 ${W} ${H}`} role="img"
          aria-label="Illustrative Spate sink stall: the bounded queue is full, source lanes are paused, source event polling continues, and commits wait for acknowledgements.">
          {FIGURE.map((piece) => piece.still)}
        </svg>
        <figcaption>
          <b>Spate · illustrative sink stall</b>
          A full queue pauses source lanes while source event polling continues. The bad-record scenario uses the explicit Skip policy.
        </figcaption>
      </figure>
      <dl className="incident__events">
        <div>
          <dt>Sink stall</dt>
          <dd>The queue fills to its bound. Source lanes pause while source event polling continues; intake resumes when pressure clears.</dd>
        </div>
        <div>
          <dt>Bad record · Skip policy</dt>
          <dd>The failed record is counted and skipped. Choosing Fail instead stops the pipeline.</dd>
        </div>
        <div>
          <dt>Shutdown</dt>
          <dd>Intake stops. Pending work drains until completion or the configured deadline; only acknowledged data is committed.</dd>
        </div>
      </dl>
    </div>
  );
